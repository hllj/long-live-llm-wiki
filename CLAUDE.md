...
An LLM-maintained personal wiki about **Large Language Models**. You (Claude) own the `wiki/` layer entirely — creating, updating, and cross-referencing markdown pages as sources are ingested and questions are asked. The human curates sources and directs analysis; you do all the bookkeeping.

**Topic:** Research, architectures, training techniques, and applications of LLMs.
...
This wiki uses **qmd** (`npm install -g @tobilu/qmd`) as its local search engine. The `large-language-models` collection is pre-configured. Key commands:

> **Project-local index:** Run all qmd commands from the repo root. If a `.qmd/` folder exists there (created by `wiki-init`), qmd automatically uses the project-local index instead of the global one. Check with `ls .qmd/` — if missing, run `qmd init` then `qmd collection add ./wiki --name large-language-models` to register the collection.

```bash
qmd query "<question>" --collection large-language-models --json --limit 8 2>/dev/null || true   # hybrid search + LLM re-ranking
qmd search "<term>"   --collection large-language-models --json --limit 10 2>/dev/null || true   # fast keyword-only search
qmd vsearch "<term>"  --collection large-language-models --json --limit 8 2>/dev/null || true    # semantic/vector search
qmd get <file>        --collection large-language-models 2>/dev/null || true                     # retrieve a specific page
qmd status            --collection large-language-models 2>/dev/null || true                     # index health
qmd update            --collection large-language-models 2>/dev/null || true                     # re-index after changes
qmd embed             --collection large-language-models 2>/dev/null || true                     # regenerate vectors after bulk changes
```
...
2. Run qmd impact scoring to find which existing pages are most affected:
   ```bash
   qmd query "<title or core claim>" --collection large-language-models --json --limit 10
   ```
...
7. Run `qmd update --collection large-language-models` to re-index all changes.
...
1. Run `qmd query "<question>" --collection large-language-models --json --limit 8`.
...
4. If filed: update `wiki/index.md`, run `qmd update --collection large-language-models`, and append to `wiki/log.md`:
   ```
...
1. Run `qmd status --collection large-language-models` for index health.
...
4. Fix issues if asked; run `qmd update --collection large-language-models` after fixes.
...
This wiki covers **Large Language Models** — research, architectures, training techniques, and applications of LLMs. The human decides which sources to ingest and what questions to ask. At the start of a new sub-topic area, create stub pages for key entities and concepts so future ingests have somewhere to land.
